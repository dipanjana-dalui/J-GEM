...
GEM_type = [1]
# GEM_ver = [1] ## another upper level loop if needed - will introduce another for loop
...
no_param = length(params)  ## also, size(state_par_match, 2)

# The following never get mutated, hence a good place to use StaticArrays
state_par_match = SMatrix{no_species, no_param}(1, 1, 1, 1)
state_geno_match = SMatrix{no_species, no_param}(0, 0, 0, 0)
geno_par_match = SMatrix{no_species, no_param}(0, 0, 0, 0)
...
pop_stand_out_all = fill(NaN, no_species, num_time_steps, num_rep, length(GEM_type))
x_stand_out_all = fill(NaN, no_columns-1,num_time_steps, no_species,num_rep, length(GEM_type))
x_var_stand_out_all = fill(NaN, no_columns-1,num_time_steps, no_species,num_rep, length(GEM_type))
...
#    j = 1 # for debugging
 ...
 #       i=1
        ...
        for ii = 1:no_species #8×21×1 Array{Float64, 3}:[:, :, 1] 
            #ii = 1
            x_slice[:, 1, ii] = CalcAverageFreqs(ii, no_columns, no_param, x_dist_init)
            x_var_slice[1:no_param,1,ii] = mapslices(var ∘ skipmissing,x_dist_init[x_dist_init[:,1] .== ii,2:no_param+1],dims=1)
        ...
        while t < t_max  && 1 <=  sum(R)  
            ...
            # Find who is next
            FindWhoNext = WhoIsNext(x_dist,no_species,no_columns,no_param,R,
            ...
            if row == 1 && col == 1
                ...
                new_trait = DrawNewTraits(x_dist,parent_traits,h2_vect,no_param,no_columns,col)
            ...
            if t > time_step
                ...
                for ii in 1:no_species
                    x_slice[:,time_step_index,ii] = CalcAverageFreqs(ii,no_columns,no_param,x_dist)
                    x_var_slice[1:no_param,time_step_index,ii] = mapslices(var∘ skipmissing, x_dist[x_dist[:,1].==ii,2:no_param+1],dims=1)
...
out_mat = reshape(out, 51, 5)
out_dat = DataFrame(out_mat, :auto)
dat = hcat(stand_time, out_dat, makeunique=true)
